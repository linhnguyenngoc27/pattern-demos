...
public class ZipFileCtrl {
	...
	public StreamedContent downloadZipFile() {
		try {
			var inputStream = new FileInputStream(this.zipFile.getJavaFile());
			var streamedContent = DefaultStreamedContent.builder().name(this.zipFile.getName())
	...
	public void onUnzipFile() {
		...
		try {
			var outFolder = new File(folderName);
			this.unzipFolder = outFolder.getAbsolutePath();

			ZipService.get().unzip(zipFile, this.unzipFolder);
		} catch (IOException e) {
			Ivy.log().error("Error when unzip file {0} is error {1}", e, this.zipFile.getName(), e.getMessage());
		}
	}

	public String getTotalSize() {
		Long size = emptyIfNull(this.zipModels).stream().map(ZipModel::getSize).reduce(0L, Long::sum);
		return byteCountToDisplaySize(ofNullable(size).orElse(0L));
	}

	public String getTotalCompressedSize() {
		Long size = emptyIfNull(this.zipModels).stream().map(ZipModel::getCompressedSize).reduce(0L,
				Long::sum);
		return byteCountToDisplaySize(ofNullable(size).orElse(0L));
	}

	public String getFileSizeDisplay() {
		return byteCountToDisplaySize(ofNullable(this.zipFile.getJavaFile().length()).orElse(0L));
	}
	...
	private File createZipFile(List<Entry<String, byte[]>> data) {
		try {
			...
			var file = new File(UUID.randomUUID().toString() + ".zip");
